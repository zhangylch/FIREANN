...
class Calculator():
    ...
    def get_dipole(self,cell,disp_cell,cart,ef,index_cell,neigh_list,shifts,species,create_graph=False):
        atomic_energy=self.pes(cell,disp_cell,cart,ef,index_cell,neigh_list,shifts,species)
        varene=torch.sum(atomic_energy)
        dipole=torch.autograd.grad(varene,ef,create_graph=create_graph)[0]
        return dipole,

    def get_force(self,cell,disp_cell,cart,ef,index_cell,neigh_list,shifts,species,create_graph=False):
        atomic_energy=self.pes(cell,disp_cell,cart,ef,index_cell,neigh_list,shifts,species)
        varene=torch.sum(atomic_energy)
        force = -torch.autograd.grad(varene,cart,create_graph=create_graph)[0]
        return force,

    def get_ene_dipole(self,cell,disp_cell,cart,ef,index_cell,neigh_list,shifts,species,create_graph=False):
        atomic_energy=self.pes(cell,disp_cell,cart,ef,index_cell,neigh_list,shifts,species)
    ...
    def get_ene(self,cell,disp_cell,cart,ef,index_cell,neigh_list,shifts,species,create_graph=False):
        atomic_energy=self.pes(cell,disp_cell,cart,ef,index_cell,neigh_list,shifts,species)
        varene=torch.sum(atomic_energy.view(-1,cart.shape[1]),dim=1)
        return varene,
    
    def get_pol(self,cell,disp_cell,cart,ef,index_cell,neigh_list,shifts,species,create_graph=False):
        atomic_energy=self.pes(cell,disp_cell,cart,ef,index_cell,neigh_list,shifts,species)
        varene=torch.sum(atomic_energy)
    ...
    def get_stresstensor(self,cell,disp_cell,cart,ef,index_cell,neigh_list,shifts,species,create_graph=False):
        atomic_energy=self.pes(cell,disp_cell,cart,ef,index_cell,neigh_list,shifts,species)
        varene=torch.sum(atomic_energy.view(-1,cart.shape[1]),dim=1)
        stress=torch.autograd.grad(torch.sum(varene),disp_cell,create_graph=create_graph)[0]/(cell[:,0,0]*cell[:,1,1]*cell[:,2,2])
        return stress

    def get_ene_stress(self,cell,disp_cell,cart,ef,index_cell,neigh_list,shifts,species,create_graph=False):
        atomic_energy=self.pes(cell,disp_cell,cart,ef,index_cell,neigh_list,shifts,species)
        varene=torch.sum(atomic_energy.view(-1,cart.shape[1]),dim=1)
        stress=torch.autograd.grad(torch.sum(varene),disp_cell,create_graph=create_graph)[0]/(cell[:,0,0]*cell[:,1,1]*cell[:,2,2])
