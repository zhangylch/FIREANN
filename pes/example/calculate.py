...
class Calculator():
    ...
    def get_dipole(self,cell,cart,ef,neigh_list,shifts,species,create_graph=False):
        atomic_energy=self.pes(cell,cart,ef,neigh_list,shifts,species)
        varene=torch.sum(atomic_energy)
        dipole=torch.autograd.grad(varene,ef,create_graph=create_graph)[0]
        return dipole,

    def get_ene_dipole(self,cell,cart,ef,neigh_list,shifts,species,create_graph=False):
        atomic_energy=self.pes(cell,cart,ef,neigh_list,shifts,species)
    ...
    def get_ene(self,cell,cart,ef,neigh_list,shifts,species,create_graph=False):
        atomic_energy=self.pes(cell,cart,ef,neigh_list,shifts,species)
        varene=torch.sum(atomic_energy.view(-1,cart.shape[1]),dim=1)
        return varene,
    
    def get_pol(self,cell,cart,ef,neigh_list,shifts,species,create_graph=False):
        atomic_energy=self.pes(cell,cart,ef,neigh_list,shifts,species)
        varene=torch.sum(atomic_energy)
    ...
    def get_stresstensor(self,cell,cart,ef,neigh_list,shifts,species,create_graph=False):
        atomic_energy=self.pes(cell,cart,ef,neigh_list,shifts,species)
        varene=torch.sum(atomic_energy.view(-1,cart.shape[1]),dim=1)
        stress=torch.autograd.grad(torch.sum(varene),cell,create_graph=create_graph)[0]
        return stress

    def get_ene_stress(self,cell,cart,ef,neigh_list,shifts,species,create_graph=False):
        atomic_energy=self.pes(cell,cart,ef,neigh_list,shifts,species)
        varene=torch.sum(atomic_energy.view(-1,cart.shape[1]),dim=1)
        stress=torch.autograd.grad(torch.sum(varene),cell,create_graph=create_graph)[0]
