...
lr_average=5e-5                # learning rate for starting swa
...
numpoint,atom,mass,numatoms,scalmatrix,period_table,coor,ef,abprop,force=Read_data(floderlist,Prop_list,start_table=start_table)
...
for i,iprop in enumerate(Prop_list):
    if iprop == "Force":
        ...
        abprop_test+=(force_test,)
        train_nele.append(np.sum(numatoms[0:numpoint[0]])*3)
        test_nele.append(np.sum(numatoms[numpoint[0]:ntotpoint])*3)
        tmp_f=1
    else:
        abprop_train+=(torch.from_numpy(abprop[i-tmp_f][range_train[0]:range_train[1]]).to(torch_dtype),)
        abprop_test+=(torch.from_numpy(abprop[i-tmp_f][range_test[0]:range_test[1]]).to(torch_dtype),)
        train_nele.append(numpoint[0]*Prop_dict[Prop_list[i]])
        test_nele.append(numpoint[1]*Prop_dict[Prop_list[i]])
ef_train=torch.from_numpy(np.array(ef[range_train[0]:range_train[1]],dtype=np_dtype))
ef_test=torch.from_numpy(np.array(ef[range_test[0]:range_test[1]],dtype=np_dtype))
train_nele=torch.from_numpy(np.array(train_nele)).to(device)
test_nele=torch.from_numpy(np.array(test_nele)).to(device)
