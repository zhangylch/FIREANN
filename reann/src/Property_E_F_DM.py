...
class Property(torch.nn.Module):
    ...
    def forward(self,cart,ef,numatoms,species,atom_index,shifts,create_graph=True):
        ...
        force,dipole=torch.autograd.grad(torch.sum(varene),(cart,ef),\
        create_graph=create_graph,only_inputs=True,allow_unused=True)
        return (varene.view(-1,1),dipole.view(numatoms.shape[0],-1),-force.view(numatoms.shape[0],-1))
