...
PairFIREANN::PairFIREANN(LAMMPS *lmp) : Pair(lmp) 
{
}

PairFIREANN::~PairFIREANN() 
{
    if (allocated) 
    {
...
void PairFIREANN::init_style()
{
    ...
    ef=ef.to(tensor_type);
    module.to(tensor_type);
    module.eval();
    // freeze the module
    module=torch::jit::optimize_for_inference(module);
    
    cout << "Load model correctly" << endl;
...
void PairFIREANN::coeff(int narg, char **arg)
{
    if (!allocated) 
    {
        allocate();
    ...
    ef[0] = utils::numeric(FLERR, arg[4], false,lmp);
    ef[1] = utils::numeric(FLERR, arg[5], false,lmp);
    ef[2] = utils::numeric(FLERR, arg[6], false,lmp);
    datatype=arg[3];
...
void PairFIREANN::settings(int narg, char **arg)
{
}


double PairFIREANN::init_one(int i, int j)
{
  return cutoff;
}
...
void PairFIREANN::compute(int eflag, int vflag)
{
    ...
    auto outputs = module.forward({cart_,ef,atom_index_,local_species_,neigh_list_}).toTuple()->elements();
